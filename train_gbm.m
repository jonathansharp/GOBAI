...
% gradient boosted machines in GMM clusters and predict
% on a grid.
%
...
gbm_dir = ['Models/' base_grid '/GBM/c' num2str(num_clusters) '_' file_date ...
...
for c = 1:num_clusters
  ...
  if any(all_data_clusters.clusters == c)
    ...
    % stop timing fit
    fprintf(['Train GBM - Cluster #' num2str(c) ': N/A']);
    [~]=toc;
...
parfor m = 1:timesteps
    ...
    if strcmp(base_grid,'RG')
        ...
        apply_gbm_model(TS,num_clusters,gbm_dir,gbm_fnames,...
            base_grid,m,1,xdim,ydim,zdim,variables_TS,thresh,gobai_gbm_dir);
    elseif strcmp(base_grid,'RFROM')
...
function apply_gbm_model(TS,num_clusters,gbm_dir,gbm_fnames,...
    ...
    for c = 1:num_clusters
      ...
      if isfile([gbm_dir '/' gbm_fnames{c}])
