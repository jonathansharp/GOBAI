...
if exist([param_props.dir_name '/Data/processed_wod_' param_props.file_name '_data_' year '.mat'],'file') ~= 2
...
parfor y = start_year:end_year
  if ~isfile([folder '/wod_' param_props.file_name '_interpolated_' num2str(y)])
    try
        % define variables and preallocate
        dims = {'wod_unique_cast' 'WOD_cruise_identifier' 'time' 'date' 'lat' 'lon'};
        if strcmp(param_props.file_name,'o2')
            vars = {'Temperature' 'Salinity' 'z' var_name};
        elseif strcmp(param_props.file_name,'no3')
            vars = {'Temperature' 'Salinity' 'z' 'Oxygen' var_name};
        end
        vars_both = [dims vars];
        wod = struct();
        ...
        % loop through for each data type
        for x = 1:length(types)
            file = [folder '/wod_' types{x} '_' num2str(y) '.nc'];
            if exist(file,'file')
                schema = ncinfo(file);
                ...
                for d = 1:length(dims)
                    ...
                    if strcmp(dims{d},'WOD_cruise_identifier')
                        wod_temp.(dims{d}) = wod_temp.(dims{d})';
                    end
                end
                ...
                % add type (1 = osd and 2 = ctd)
                if strcmp(types{x},'osd')
                    wod.type = [wod.type;ones(length(wod_temp.Temperature_common(idx)),1)];
                elseif strcmp(types{x},'ctd')
                    wod.type = [wod.type;repmat(2,length(wod_temp.Temperature_common(idx)),1)];
                end
        ...
        wod.date = datevec(wod.time);
        wod.year = wod.date(:,1);
        wod.month = wod.date(:,2);
        wod.day = wod.date(:,3);
        ...
        for f = 1:length(stations) % for each unique station id
            ...
            idx = wod.wod_unique_cast == stations(f);

            %% interpolate (if profile doesn't overlap with a glodap profile)
            % same day and within about 5km as a glodap profile
            overlap_idx_glodap = any(abs(glodap_data.LAT-mean(wod.lat(idx))) < 0.05 & ...
                abs(glodap_data.LON-mean(wod.lon(idx))) < 0.05 & ...
                abs(glodap_data.TIME-mean(wod.time(idx))) < 1);

            if ~overlap_idx_glodap

                if unique(wod.type(idx)) == 2
                    % same day and within about 5km as a bottle profile
                    overlap_idx_bottle = any(abs(wod.lat(wod.type==1)-mean(wod.lat(idx))) < 0.05 & ...
                        abs(wod.lon(wod.type==1)-mean(wod.lon(idx))) < 0.05 & ...
                        abs(wod.time(wod.type==1)-mean(wod.time(idx))) < 1);
                elseif unique(wod.type(idx)) == 1
                    overlap_idx_bottle = false;
                end

                if ~overlap_idx_bottle
                    ...
                    wod_data.CRU = [wod_data.CRU;...
                        repmat(mean(wod.WOD_cruise_identifier(idx)),length(zi),1)];
                    wod_data.ID = [wod_data.ID;...
                        repmat(mean(wod.wod_unique_cast(idx)),length(zi),1)];
                    wod_data.TYPE = [wod_data.TYPE;...
                        repmat(mean(wod.type(idx)),length(zi),1)];
        ...
        idx = true(size(wod_data.TEMP));
        for v = 1:length(varsi)
           idx(isnan(wod_data.(varsi{v}))) = false;
        end
        vars = fieldnames(wod_data);
        for v = 1:length(vars)
            wod_data.(vars{v}) = wod_data.(vars{v})(idx);
        end

        % save year of interpolated data
        parsave([folder '/wod_interpolated_' num2str(y)],wod_data,'wod_data');

    catch ME
        fprintf(['Error in iteration for ' num2str(y) ', Line ' num2str(ME.stack(end).line) ': ' ME.message '\n']);
    end
...
for y = start_year:end_year
    ...
    wod_temp = load([folder '/wod_interpolated_' num2str(y)],'wod_data');
    % add to structure
    for v = 1:length(vars)
        wod_data.(vars{v}) = [wod_data.(vars{v});wod_temp.wod_data.(vars{v})];
    end
end
...
exportgraphics(gcf,[param_props.dir_name '/Figures/Data/processed_wod_filtered_' year '.png']);
close
clear lon_temp
...
% m_coast('patch',rgb('gray'));
% m_grid('linestyle','-','xticklabels',[],'yticklabels',[],'ytick',-90:30:90);
% idx_gl = glodap_data.YEAR == 2016;
% lon_temp = convert_lon(convert_lon(glodap_data.LON(idx_gl)),'0-360');
% lon_temp(lon_temp < 20) = lon_temp(lon_temp < 20) + 360;
% m_scatter(lon_temp,glodap_data.LAT(idx_gl),'.g');
% 
% 
% figure; hold on;
% set(gcf,'visible','on','position',[100 100 1600 800]);
...
% m_coast('patch',rgb('gray'));
% m_grid('linestyle','-','xticklabels',[],'yticklabels',[],'ytick',-90:30:90);
% idx_wo = wod_data.YEAR == 2016 & wod_data.TYPE == 2;
% lon_temp = convert_lon(convert_lon(wod_data.LON(idx_wo)),'0-360');
% lon_temp(lon_temp < 20) = lon_temp(lon_temp < 20) + 360;
% m_scatter(lon_temp,wod_data.LAT(idx_wo),'.g');
...
save([param_props.dir_name '/Data/processed_wod_' param_props.file_name ...
    '_data_' year '.mat'],'wod_data','-v7.3');
