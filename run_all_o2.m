...
%% average rfrom to climatology and 1x1 monthly
RFROM_climatology_temp(rfrom_ver,start_year,end_year);
RFROM_climatology_sal(rfrom_ver,start_year,end_year);
% average_rfrom_files(fpaths,rfrom_ver,start_year,end_year); % this doesn;t work because I don't have write permission (I think)

%% plot rfrom animation
plot_rfrom_animation(fpaths,'temp',rfrom_ver,'RFROM',...
    start_year,end_year,-1,20,1,'projection','polar','plot_year',2020);
plot_rfrom_animation(fpaths,'sal',rfrom_ver,'RFROM',...
    start_year,end_year,-1,20,1,'projection','polar','plot_year',2020);
...
% adjust data
adjust_o2_data(float_file_ext,glodap_vrs,snap_date,flt,gld,osd,ctd);
% display data
display_data(param_props,float_file_ext,glodap_vrs,...
    start_year,snap_date,flt,gld,osd,ctd);
% combine data (float,glodap,ctd)
combine_data(param_props,float_file_ext,...
    glodap_vrs,snap_date,flt,gld,osd,ctd);
...
%     datetime('today','Format','dd-MMM-uuuu')],'kfold_table');
...
predict_gobai('FFNN',param_props,fpaths,base_grid,file_date,float_file_ext,...
    ...
    'test_ratio',test_ratio,'reduce_data',data_per);
...
%     file_date,snap_date,glodap_vrs,float_file_ext,start_year,end_year,...
...
%     model_types,num_clusters,numWorkers_predict,file_date,snap_date,float_file_ext,...
%     glodap_vrs,train_ratio,val_ratio,test_ratio,flt,gld,osd,ctd);
% plot_gobai_animation(param_props,fpaths,base_grid,num_clusters,'FFNN',...
%     file_date,float_file_ext,numWorkers_predict,flt,gld,ctd,...
%     'train_ratio',train_ratio,'val_ratio',val_ratio,'test_ratio',test_ratio,'uncer',1);
