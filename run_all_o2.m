...
data_per_osse = 1; % set data reduction to 10% for osse
param = 'o2';
...
% RFROM_climatology_temp('v2.2_2025',start_year,end_year);
% RFROM_climatology_sal('v2.2_2025',start_year,end_year);
% average_rfrom_files(fpaths,'v2.2_2025',start_year,end_year);
...
if osd == 1 | ctd == 1; acquire_wod_data(param_props,...
    glodap_year,start_year,osd,ctd,numWorkers_predict); end
% adjust data
adjust_o2_data(float_file_ext,glodap_year,...
    snap_date,flt,gld,osd,ctd);
% display data
...
combine_data(param_props,float_file_ext,...
    glodap_year,snap_date,flt,gld,osd,ctd);

%% plot histogram of data
plot_data_hist(param_props,file_date,start_year,end_year,float_file_ext,...
    flt,gld,osd,ctd,start_year,end_year);
...
run_osse(fpaths,model_types,model_folders,realizations,grid_labels,...
    ...
    thresh,data_per_osse,numWorkers_train,numWorkers_predict,flt,gld,osd,ctd);
...
%     glodap_year,train_ratio,val_ratio,test_ratio,flt,gld,osd,ctd);
% plot_gobai_animation(param_props,fpaths,base_grid,num_clusters,'FFNN',...
