...
%% Plot RFROM Salinity over time

function plot_rfrom_sal_animation(fpaths,ver,base_grid,start_year,end_year)
...
parfor d = 1:length(pressures)
    ...
    for m = 1:length(TS.months)
        % establish file name
        fname = ['rfrom_sal_animation_' num2str(pressures(d)) 'dbar.gif'];
        % load dimensions
        TS = load_RFROM_dim(fpaths.temp_path,start_year,end_year);
        % process longitude
        ...
        nc_atts = ncinfo([fpaths.sal_path 'RFROM_SAL_v2.2/RFROMV22_SAL_STABLE_' ...
        ...
        for w = 1:nc_atts.Dimensions(3).Length
            ...
            cnt = TS.cnt{m}(w);
            % get RFROM T
            rfrom = ncread([fpaths.sal_path 'RFROM_SAL_v2.2/RFROMV22_SAL_STABLE_' ...
                num2str(TS.years(m)) '_' sprintf('%02d',TS.months(m)) '.nc'],...
            ...
            export_fig(h,[dname '/' num2str(pressures(d)) ...
                'dbars/sal_t' num2str(cnt) '.png'],'-transparent','-silent');
            % create folder
    ...
    close
    % display information
    disp(['RFROM Salinity animation at ' num2str(pressures(d)) ' dbar plotted'])
end
