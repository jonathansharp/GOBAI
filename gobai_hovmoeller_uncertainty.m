...
gobai_uncer_3d = reshape(GOBAI.uncer,[length(GOBAI.lon)*length(GOBAI.lat) length(GOBAI.pres) length(GOBAI.time)]);
...
global_mean_uncer_2d = sum(gobai_uncer_3d.*gobai_vol_2d,'omitnan')./sum(gobai_vol_2d,'omitnan');
global_mean_uncer_2d = squeeze(global_mean_uncer_2d);
...
% initialize figure for absolute uncertainty
figure;
set(gcf,'units','inches','position',[0 0 20 10]); hold on;
set(gca,'fontsize',26,'YDir','reverse','linewidth',3,...
    'layer','top','box','on','CLim',[5 12]);
title('GOBAI-O_{2} Global Mean [O_{2}] Uncertainty Over Time (\mumol kg^{-1})','fontsize',24);

% plot data
contourf(time_axis,double(GOBAI.pres),...
    double(global_mean_uncer_2d),5:0.5:12);
...
% save figure
exportgraphics(gcf,['Figures/global_O2_uncertainty_hovmoeller_GOBAI_' ver '.png']);
close

% initialize figure for anomaly
figure;
set(gcf,'units','inches','position',[0 0 20 10]); hold on;
set(gca,'fontsize',26,'YDir','reverse','linewidth',3,'layer','top','box','on','CLim',[-0.95 0.95]);
title('GOBAI-O_{2} Global Mean [O_{2}] Uncertainty Anomaly Over Time (\mumol kg^{-1})','fontsize',24);

% plot data
contourf(time_axis,double(GOBAI.pres),...
    double(global_mean_uncer_2d-mean(global_mean_uncer_2d,2)),-0.95:0.1:0.95);
...
% save figure
exportgraphics(gcf,['Figures/global_O2_uncertainty_anom_hovmoeller_GOBAI_' ver '.png']);
close
...
% plot data
global_mean_uncer = sum(global_mean_uncer_2d.*...
    squeeze(GOBAI.h(1,1,:)))./sum(squeeze(GOBAI.h(1,1,:)));
plot(time_axis,global_mean_uncer,'k','linewidth',3);
ylabel('{\itu}[O_{2}] (\mumol kg^{-1})','FontSize',21);
yyaxis right
...
% save figure
exportgraphics(gcf,['Figures/global_O2_uncertainty_wfloats_GOBAI_' ver '.png']);
close
clear
