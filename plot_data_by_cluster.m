...
function plot_data_by_cluster(param_props,base_grid,file_date,...
    float_file_ext,num_clusters,numWorkers_predict,flt,gld,osd,ctd,varargin)
...
if strcmp(base_grid,'RG') || strcmp(base_grid,'RFROM')
    ...
    load([param_props.dir_name '/Data/GMM_' base_grid '/' ...
        ...
        coverage '.mat'],'all_data_clusters');
end
...
parfor p = 1:length(pressures)
    ...
    export_fig(gcf,[dname '/clustered_data' coverage '_' float_ext glodap_ext osd_ext ctd_ext ...
    ...
    for clst = 1:num_clusters
        ...
        dname = [param_props.dir_name '/Figures/Clusters/' base_grid '_c' num2str(num_clusters)];
        if ~isfolder([pwd '/' dname]); mkdir(dname); end
        export_fig([dname '/clustered_data_probability' ...
            coverage '_c' num2str(clst) '_' float_ext glodap_ext ...
            osd_ext ctd_ext '_' num2str(pressures(p)) '.png'],...
            '-transparent','-silent');
        close
    end
end
