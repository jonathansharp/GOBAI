...
pressure_level = 200;
lb_sz = 6;
map_projection = 'pcarree';

% set plot properties
regions = {'GS','KUR','EAS','BRA'};
lat = [33 53.5; 28 48.5; -30 -9.5; -55 -34.5];
lon = [295 325.5; 145 175.5; 145 175.5; 285 315.5];
...
% establish figure
figure('Position',[100 100 1000 800],'visible','on');
...
for r = 1:length(regions)
    ...
    if r == 4
        ...
        oxy_temp1 = double(ncread([path2 'GOBAI-' var1 '-HR-v' date_ver '.nc'],'o2',...
            [min(idx_lon_2_1) min(idx_lat_2) idx_pres_2 idx_time_2],...
            [length(idx_lon_2_1) length(idx_lat_2) 1 1]));
        oxy_temp2 = double(ncread([path2 'GOBAI-' var1 '-HR-v' date_ver '.nc'],'o2',...
            [min(idx_lon_2_2) min(idx_lat_2) idx_pres_2 idx_time_2],...
        ...
        nit_temp1 = double(ncread([path3 'GOBAI-' var2 '-HR-v' date_ver '.nc'],'no3',...
            [min(idx_lon_2_1) min(idx_lat_2) idx_pres_3 idx_time_3],...
            [length(idx_lon_2_1) length(idx_lat_2) 1 1]));
        nit_temp2 = double(ncread([path3 'GOBAI-' var2 '-HR-v' date_ver '.nc'],'no3',...
            [min(idx_lon_2_2) min(idx_lat_2) idx_pres_3 idx_time_3],...
            [length(idx_lon_2_2) length(idx_lat_2) 1 1]));
        ...
        gobai2.oxy = double(ncread([path2 'GOBAI-' var1 '-HR-v' date_ver '.nc'],'o2',...
    ...
    setm(axis.(['ax1_' num2str(r)]),'MapProjection',map_projection,'FontSize',12);
    mlabel('south');
    ...
    worldmap(lat(r,:),lon(r,:));
    setm(axis.(['ax2_' num2str(r)]),'MapProjection',map_projection,'FontSize',12);
    ...
    worldmap(lat(r,:),lon(r,:));
    setm(axis.(['ax3_' num2str(r)]),'MapProjection',map_projection,'FontSize',12);
    mlabel('south'); plabel('off');
...
colormap(axis.colorbar1,cmocean('ice'));
clim(axis.colorbar1,[o2_lims(r,1) o2_lims(r,2)]);
...
colormap(axis.colorbar2,cmocean('speed'));
clim(axis.colorbar2,[nit_lims(r,1) nit_lims(r,2)]);
...
exportgraphics(gcf,['Figures/Figure4_' num2str(pressure_level) '_' datestr(time) '_new_regions.png']);
close