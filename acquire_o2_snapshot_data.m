...
if exist(['Data/processed_float_o2_data_' file_date float_file_ext '.mat'],'file') ~= 2

%% Define file structure
fpath = ['BGC_Argo_Snapshots/' num2str(snap_date) '-BgcArgoSprof/dac']; % define file path
folderinfo = dir(fpath); % extract info about folders and files
...
for n = 1:length(idx_folders) % for each DAC
    ...
    %% Define file info
    fileinfo = dir([fpath '/' foldernames{idx_folders(n)} '/*.nc']); % extract float file info
    ...
    for f = 1:length(filenames) % for each float
        ...
        float.(['F' floatnum]) = ... % download float data
            netcdfreader([fpath '/' foldernames{idx_folders(n)} '/' filenames{f}]);
        [n_prof, n_param, n_levels] = ... % get dimensions
            get_dims([fpath '/' foldernames{idx_folders(n)} '/' filenames{f}]);
...
%% clean up
clear foldernames fpath idx_folders n vars zi
...
save(['Data/processed_float_o2_data_' file_date float_file_ext '.mat'],...
