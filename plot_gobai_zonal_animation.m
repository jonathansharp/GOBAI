...
function plot_gobai_zonal_animation(param_props,param_path,base_grid,num_clusters,...
...
for l = 1:length(latitudes)
    ...
    % establish file name
    fname = ['gobai_transect_animation_' num2str(latitudes(l)) 'deg.gif'];
    ...
    % load dimensions
    Longitude = ncread(gobai_fname,'lon',721,360);
    Latitude = ncread(gobai_fname,'lat');
    Pressure = ncread(gobai_fname,'pres',1,20);
    % process longitude
%     idx_20 = Longitude<20;
%     Longitude(idx_20) = Longitude(idx_20)+360;
%     Longitude = [Longitude(~idx_20);Longitude(idx_20)];
    % latitude index
    [~,lat_idx] = min(abs(Latitude-latitudes(l)));
    % establish fiugre
    h = figure('color','w','visible','on','units','inches','Position',[0 0 3.6*4 1.7*4]);
    axis tight manual
    ...
    for t = 1:timesteps
        ...
        gobai = squeeze(ncread(gobai_fname,param_props.file_name,...
            [721 lat_idx 1 t],[360 1 20 1]));
        time = ncread(gobai_fname,'time',t,1)+datenum(1950,0,0);
        % establish figure
        figure(h);
        % make plot
        %pcolor(double(Latitude),double(Pressure),double(gobai)'); shading flat;
        contourf(double(Longitude),double(Pressure),double(gobai)');
        xlabel('Longitude'); ylabel('Depth (dbar)');
        if strcmp(base_grid,'RFROM')
            title(gca,[param_props.label ' ' param_props.units ', ' datestr(time,'mmm-YYYY')],'FontSize',24);
        else
            title(gca,extractAfter(datestr(datenum(2004,t,1)),'-'),'FontSize',24);
        end
        ...
        c.TickLength = 0;
%         % save frame
%         if ~isfolder([dname '/' num2str(latitudes(l)) 'deg'])
%             mkdir([dname '/' num2str(latitudes(l)) 'deg']);
%         end
%         export_fig(h,[dname '/' num2str(latitudes(l)) ...
%             'deg/t' num2str(t) '.png'],'-transparent','-silent');
    ...
    close
    % display information
    disp(['GOBAI-' param_props.dir_name ' (' alg_type ') animation at ' num2str(latitudes(l)) ' deg plotted'])
end
