...
grid_uncer.float_oxy_std = single(accumarray(subs_float(idx_float,:),float_data.OXY(idx_float),sz,@nanstd,nan));
grid_uncer.glodap_oxy_std = single(accumarray(subs_glodap(idx_glodap,:),glodap_data.OXY(idx_glodap),sz,@nanstd,nan));
grid_uncer.oxy_std = mean(cat(5,grid_uncer.float_oxy_std,grid_uncer.glodap_oxy_std ),5,'omitnan');
grid_uncer = rmfield(grid_uncer,{'float_oxy_std' 'glodap_oxy_std'});
grid_uncer.idx = ~isnan(grid_uncer.oxy_std); % index when oxygen variability exists
grid_uncer.oxy_std = double(grid_uncer.oxy_std(grid_uncer.idx));
% get mean within each grid cell (sigma)
grid_uncer.float_sig = single(accumarray(subs_float(idx_float,:),float_data.OXY_SIGMA(idx_float),sz,@nanmean,nan));
grid_uncer.glodap_sig = single(accumarray(subs_glodap(idx_glodap,:),glodap_data.OXY_SIGMA(idx_glodap),sz,@nanmean,nan));
grid_uncer.sig = mean(cat(5,grid_uncer.float_sig,grid_uncer.glodap_sig),5,'omitnan');
grid_uncer = rmfield(grid_uncer,{'float_sig' 'glodap_sig'});
grid_uncer.sig = double(grid_uncer.sig(grid_uncer.idx));
...
mod = fitlm(matched_data.OXY_FLOAT,matched_data.OXY_DELTA,'Intercept',false); % direct fit
% mod = fitlm(matched_data.OXY_FLOAT,100.*(matched_data.OXY_DELTA./matched_data.OXY_FLOAT),'Intercept',true); % percent fit
slp = mod.Coefficients.Estimate(1);
int = 0; % mod.Coefficients.Estimate(1);
% apply linear correction to float O2
matched_data.OXY_FLOAT_CORR = matched_data.OXY_FLOAT + slp.*matched_data.OXY_FLOAT + int; % direct correction
% matched_data.OXY_FLOAT_CORR = matched_data.OXY_FLOAT + ((slp.*matched_data.OXY_FLOAT + int)./100).*matched_data.OXY_FLOAT; % percent correction
% re-calculate delta
